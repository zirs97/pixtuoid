...
- "How does config persistence work?" → `config.rs` defines `AppConfig` (theme, max-desks as `Option<T>`), `config_path()` (XDG-aware: `$XDG_CONFIG_HOME/ascii-agents/config.toml` or `~/.config/ascii-agents/config.toml`), `load()` (never crashes — logs warning on malformed TOML), `save()` (atomic tmp+rename, preserves existing keys), `resolve()` (CLI > config > default). Theme saved on `[t]` picker Enter confirm in `tui/mod.rs`. CLI `--theme`/`--max-desks` are `Option<T>` so config values aren't shadowed by clap defaults.
