...
- "How do overflow agents get rendered?" → agents past `max_desks` overflow to additional floors (up to `MAX_FLOORS=5`). Each floor has its own `FloorCtx` (router/cache/overlay/history) and `FloorMeta` (altitude/cat_seed/sunlight_boost/variant). `render_to_rgb_buffer` takes `FloorMeta` to drive per-floor cityview height, cat path, lighting, and decoration rotation via `SceneLayout::compute_with_seed(floor_seed)`.
...
- "Why don't old idle sessions show on startup?" → `source::jsonl::initial_seed_walk`. Checks `session_ended_last` (tail-scans last 8KB for `session_end`/`SessionEnd` markers) and skips files not modified in 5+ min. mtime > `DEFAULT_INITIAL_WINDOW` (1 hour) → cursor seeded at EOF, no `SessionStart`.
...
- "How does the neon wall display work?" → `pixel_painter/background.rs::paint_neon_panel` paints the dark panel with pulsing cyan border in the pixel buffer; `widgets.rs::paint_wall_display` overlays ratatui text (branding, state dots, scrolling ticker); `widgets.rs::TickerQueue` manages the persistent scrolling message buffer.
...
- "How does the coffee run work?" → `Pose::Walking.carrying_coffee` set in `idle_pose` walk-back from Pantry → `walking_coffee` sprite selected in pixel_painter → `has_desk_coffee()` in `pose.rs` derives desk cup state from wander cycle timing → `last_idle_at` on `AgentSlot` preserves the anchor across Active states → exit walk checks `has_desk_coffee` for carrying cup.
...
- "How do multi-floor offices work?" → `tui/floor.rs` defines `FloorCtx` (per-floor render state), `FloorTransition` (slide animation), `FloorScene` (agent projection). `tui_renderer.rs` owns `Vec<FloorCtx>` + `Vec<RgbBuffer>` and switches between them. Floor membership derived from `desk_index / max_desks`. `max_desks` is auto-computed: each frame, `tui/mod.rs` reads `cached_layout.home_desks.len()` and stores it via `fetch_max` so the reducer's `next_free_desk` tracks terminal capacity automatically (monotone — capacity only grows within a session). `next_free_desk` in `state/mod.rs` searches `0..max_desks * MAX_FLOORS`. PageUp/PageDown/↑↓/jk in `tui/mod.rs`. Slide transition composites two buffers via `flush_buffer_to_term_at_offset`.
